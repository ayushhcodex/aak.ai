...
**Answer**: *"In the current prototype, the confidence score is a heuristic based on the technology readiness level and the amount of published literature supporting the material combination. In Phase 2, we plan to implement proper Bayesian uncertainty quantification — the confidence score would then represent the posterior predictive uncertainty of the ML model, combining both epistemic uncertainty (model's lack of knowledge) and aleatoric uncertainty (inherent variability in material properties). This is a well-established technique in probabilistic machine learning."*
